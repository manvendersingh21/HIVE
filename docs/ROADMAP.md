...
- [x] `hive sessions` — every tmux session on this machine and on each configured
      worker, from `tmux list-sessions` rather than from an in-process map that a
      fresh CLI invocation would always find empty; unreachable workers are named,
      not silently dropped
- [x] `hive attach <session_id>` — finds the session locally or on a worker and
      `exec`s into it; a miss says where it looked
- [x] `hive workers <list|health>` — `health` probes every worker over SSH
- [x] `hive serve --bind` — execs the `hive-web` binary beside this one
- [ ] `hive workers <add|remove>` — **deliberately not built.** `config/workers.toml`
      is hand-maintained and its comments carry reasoning that matters (`host` is an
      SSH config alias, never a raw IP, because this repo is public — see
