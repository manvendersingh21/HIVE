...
const STOPWORDS: &[&str] = &[
    ...
    "on", "at", "by", "with", "from", "is", "are", "was", "were", "be", "been",
    "it", "its", "this", "that", "these", "those", "i", "you", "we", "they",
    "my", "your", "our", "their", "me", "him", "her", "them", "what", "which",
    "who", "when", "where", "why", "how", "do", "does", "did", "can", "could",
    "should", "would", "will", "shall", "not", "no", "yes", "about", "into",
    "over", "than", "also", "just", "some", "any", "all", "get", "got", "make",
    "made", "use", "used", "using",
];
...
pub struct MemorySystem {
    ...
    /// The chat-model client used for knowledge extraction. Local only —
    /// see `extractor.rs`'s module doc for why this must never be routed.
    extract_llm: Option<Arc<OllamaClient>>,
...
impl MemorySystem {
    ...
    fn from_parts(graph: KnowledgeGraph, config: Option<&HiveConfig>, path: &Path) -> Self {
        ...
        // Production: the embedding model and the chat model are two clients
        // over the same Ollama. `new()` (no config): the deterministic test
        // embedder and no extraction — nothing persisted is worth embedding.
        let (embed_client, rag_embedder, extract_llm): (
            Arc<OllamaClient>,
            Arc<dyn Embedder>,
            Option<Arc<OllamaClient>>,
        ) = match config {
            Some(c) => {
                let embed = Arc::new(OllamaClient::new(
                    c.llm.local.base_url.clone(),
                    c.memory.embedding_model.clone(),
                ));
                let as_embedder: Arc<dyn Embedder> = embed.clone();
                let extract = Arc::new(OllamaClient::new(
                    c.llm.local.base_url.clone(),
                    c.llm.local.model.clone(),
                ));
                (embed, as_embedder, Some(extract))
            }
            None => (
                Arc::new(OllamaClient::new("http://localhost:11434".into(), "unused".into())),
                Arc::new(rag::HashEmbedder { dim: 128 }),
                None,
            ),
        };
    ...
    pub async fn begin_turn(&self, project_id: &str, user_input: &str) -> Turn {
        ...
        let conversation = (|| -> anyhow::Result<String> {
            ...
            self.projects
                .append_message(&conv.id, "user", user_input)?;
            Ok(conv.id)
        })();
        match conversation {
            Ok(id) => Turn {
    ...
    pub async fn complete_turn(&self, conversation_id: &str, assistant_text: &str) {
        ...
        let Some(llm) = self.extract_llm.clone() else { return };
...
impl RetrievedContext {
    pub fn is_empty(&self) -> bool {
        self.rag_chunks.is_empty()
            && self.kg_entities.is_empty()
...
mod tests {
    ...
    async fn memory_with_history() -> MemorySystem {
        ...
        let t1 = mem.begin_turn("webapp", "we chose sqlite over postgres for local deploys").await;
        mem.complete_turn(&t1.conversation_id, "recorded: sqlite chosen").await;
        let t2 = mem.begin_turn("webapp", "the nightly cron job failed at 2am").await;
        mem.complete_turn(&t2.conversation_id, "fixed the cron schedule").await;
        mem
    }
    ...
    async fn retrieve_context_is_project_scoped() {
        let mem = memory_with_history().await;
        let ctx = mem.retrieve_context("other-project", "database sqlite postgres").await;
        assert!(ctx.is_empty(), "another project must not see webapp's memory");
    }
