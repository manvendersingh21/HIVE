...
impl Watchdog {
    ...
    /// Uses the configured single provider, or the local model
    /// in legacy routing mode. The supervisor polls this concurrently with
    /// terminal monitoring and drops the request when the session ends.
    pub async fn review(
        &self,
        llm: &LlmRouter,
